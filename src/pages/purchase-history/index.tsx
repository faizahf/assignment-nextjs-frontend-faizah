    ...
    <div>
      ...
      {purchases &&
        purchases.map((purchase) => (
          <div className="border my-5 rounded-xl shadow-md border-secondary bg-white p-5" key={purchase.id}>
            <div className="text-white text-center flex justify-end mb-2">
                {purchase.isPaid ? 
                    <div className="rounded-lg bg-primary px-5 py-1">
                        Paid
                    </div> 
                    :
                    <div className="rounded-lg bg-primary bg-grey px-5 py-1">
                        Unpaid
                    </div> 
                    }
            </div>
            ...
            {purchase.merchandises.length !== 0 && purchase.merchandises.map((data) => (
                <div
                ...
                <div className="flex gap-3 justify-end items-center border-t py-2">
                    <p className="flex gap-2 items-center"><MdOutlinePayments color="#7848F4" />Total Payment: </p>
                    <p className="text-primary text-2xl">{formatRupiah(purchase.paymentTotal)}</p>
                </div>
            </div>
        ))}
    </div>
